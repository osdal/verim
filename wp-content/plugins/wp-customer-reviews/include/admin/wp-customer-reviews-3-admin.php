...
class WPCustomerReviewsAdmin3 extends WPCustomerReviews3
{
	...
	function generate_css() {
		$rtn = $this->can_write_css();
		if ($rtn["can_write"]) {
			$css = $this->template('wp-customer-reviews-css');
			file_put_contents($rtn["filename"], $css, LOCK_EX); // overwrites -generated.css
		}
	}
	...
	function notice_ignore() {
		...
		if ( isset($_GET[$this->prefix.'_notice_1']) ) {
			$n1 = $_GET[$this->prefix.'_notice_1'];
			if ($n1 === 'redir') {
				wp_redirect("http://www.gowebsolutions.com/wp-customer-reviews/?from={$this->prefix}_admin_notice_1");
				exit();
			} else if ($n1 === 'ignore') {
				add_user_meta($user_id, "{$this->prefix}_admin_notice_read_1", 'true', true);
			}
		}
	}
	/* end - admin notices */
	...
	function my_add_meta_box() {
		...
		add_meta_box( $this->meta_box_posts['id'], $this->meta_box_posts['title'], array(&$this, 'show_meta_box_fields'), 'page', $this->meta_box_posts['context'], $this->meta_box_posts['priority'], array('type' => 'meta_box_posts') );
		add_meta_box( $this->meta_box_posts['id'], $this->meta_box_posts['title'], array(&$this, 'show_meta_box_fields'), 'post', $this->meta_box_posts['context'], $this->meta_box_posts['priority'], array('type' => 'meta_box_posts') );
		...
		<div class="metabox-holder">
			<div class="postbox">
				...
				<div class="inside">
					...
					<p>
						[WPCR_SHOW 
							POSTID="<span class="boldBlue">ALL</span>" 
							NUM="<span class="boldBlue">5</span>" 
							SHOWFORM="<span class="boldBlue">1</span>" 
							HIDEREVIEWS="<span class="boldBlue">0</span>" 
							HIDERESPONSE="<span class="boldBlue">0</span>" 
							SNIPPET="<span class="boldBlue"></span>" 
							MORE="<span class="boldBlue"></span>" 
							HIDECUSTOM="<span class="boldBlue">0</span>" 
						] 
						<br /><small>is available to show the latest reviews. Explanation below: <br /> 
						POSTID="<span class="boldBlue">ALL</span>" to show recent reviews from ALL posts/pages or POSTID="123" to show recent reviews from post/page ID #123<br />
						NUM="<span class="boldBlue">5</span>" will show a maximum of 5 reviews.<br />
						SHOWFORM="<span class="boldBlue">1</span>" will show the form to add a new review. This only works if POSTID is not set to "ALL".<br />
						<!--HIDEREVIEWS="<span class="boldBlue">1</span>" will hide the review output and just display the average rating stars. This only works if POSTID is not set to "ALL". <span class="boldRed">Available in Pro Version Only.</span><br />-->
						HIDERESPONSE="<span class="boldBlue">1</span>" will hide the admin response to all reviews.<br />
						SNIPPET="<span class="boldBlue">140</span>" will only show the first 140 characters of a review.<br />
						MORE="<span class="boldBlue">view more</span>" will show "... view more" with a link to the review. Only displayed when the review has been trimmed using SNIPPET.<br />
						HIDECUSTOM="<span class="boldBlue">1</span>" will hide all custom fields in the shortcode output.<br />
						</small>
					</p>
				</div>
    ...
    function admin_options() {
		...
		if ($can_write_css["can_write"] === false) {
			add_settings_error($this->prefix.'_csswrite', $this->prefix.'_csswrite', 'ERROR: You need permission (0664) to write the file '.$can_write_css["filename"], 'error');
			settings_errors($this->prefix.'_csswrite');
		}
