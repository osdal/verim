...
function wpcr3_migrate_2x_3x(&$this2, $current_dbversion) {
	...
	// begin: import per-post plugin settings
	$queryOpts = array(
		'showposts' => 9999,
		'post_type' => 'any',
	...
	$meta_query[] = array(
		'key' => 'wpcr_product_name',
		'value' => '',
		'compare' => '!='
	);
	
	$queryOpts['meta_query'] = $meta_query;
	$posts_2x = new WP_Query($queryOpts);
	$posts_2x = $posts_2x->posts;
	
	foreach ($posts_2x as $post) {
		$postid = intval($post->ID);
		
		$tmp = get_post_meta($postid);
		$meta = new stdClass();
	...
	$query = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}wpcreviews ORDER BY page_id ASC, id ASC");
